(function (window, document) {
    var SliderBar = function (targetDom, options) { 
        ...
        // 从json字符串中分离出参数
        this.options = this.extend({ //json字符串进行分离  dataList 集合 和 success 和fail两个方法
            dataList: []
        }, options);
        // 获取dom
    ...
    SliderBar.prototype = {
        ...
        extend: function (obj, obj2) { //obj2就是json字符串
            for (var k in obj2) {
                obj[k] = obj2[k];
            }
            return obj;
        ...
        mousedown: function (_this, e) {
            ...
            function mouseup() {
                ...
                if(_this.resultX > (left - 2) && _this.resultX < (left + 2)){
                    ...
                    $("#slideBar").hide();  //隐藏验证码div
					    var userName=$("#login_username").val();
                    var userPassword=$("#longin_password").val();
                    //提交账户登录表单
                      	$.ajax({
						url:"http://www.huanletao.com/user/userPsLogin",
						type:"post",
